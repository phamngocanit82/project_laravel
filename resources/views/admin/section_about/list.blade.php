<div class="card card-solid">
    ...
    @if (!empty($section_about_list))
      ...
      <div class="col-xxl-4 col-lg-6">
        ...
        <div  class="gallery my-gallery mb-1" id="aniimated-thumbnials13" itemscope="" data-pswp-uid="14">
          ...
          <ul class="ms-0 fa-ul">
            <li><div><b><h5>{{$item->title}}</h5></b></div></li>
            <li><div class="mt-2"><span>{{$item->description}}</span></div></li>
          </ul>
          <div class="mt-4 float-end">
            <input class="form-check-input" type="checkbox" id="section_about_active" name="section_about_active" onclick="activeId(this, {{$item->id}})" {!!$item->active==1? 'checked':''!!}>
            <a class="btn btn-info btn-xs" href="{{route('admin.section-about.edit', ['id'=>$item->id])}}"><i class="fas fa-pencil-alt me-1"></i>Edit</a>
  ...
  @if ($section_about_list->lastPage()>1)
    <div class="d-flex justify-content-end mt-4 me-4 mb-4">{{$section_about_list->links()}}</div>
  @endif
</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Delete this section about</h5>
...
<script>
    setTimeout(function () { 
      var Toast = Swal.mixin({
        ...
        Toast.fire({ 
          icon: 'error',  
          title:'Error edit about section about!', 
          customClass: {
            title: 'mt-2',
          }
        })
