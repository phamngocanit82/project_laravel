<div class="card card-solid">
    ...
    @if (!empty($why_us_list))
      ...
      <div class="col-xxl-4 col-lg-6">
        <div class="project-box mb-4">
          <ul class="ms-0 fa-ul">
            <li><div><h6>{{$item->title}}</h6></div></li>
            <li><div><i class="mt-2">{{$item->sub_title}}</i></div></li>
            <li><div class="mt-2"><span>{{$item->description}}</span></div></li>
          </ul>
          <div class="mt-4 float-end">
            <input class="form-check-input" type="checkbox" id="why_us_active" name="why_us_active" onclick="activeId(this, {{$item->id}})" {!!$item->active==1? 'checked':''!!}>
            <a class="btn btn-info btn-xs" href="{{route('admin.why-us.edit', ['id'=>$item->id])}}"><i class="fas fa-pencil-alt me-1"></i>Edit</a>
  ...
  @if ($why_us_list->lastPage()>1)
    <div class="d-flex justify-content-end mt-4 me-4 mb-4">{{$why_us_list->links()}}</div>
  @endif
</div>
