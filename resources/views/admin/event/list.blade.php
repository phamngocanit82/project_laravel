<div class="card card-solid">
      ...
      <div class="col-xxl-4 col-lg-6">
        <div class="project-box mb-4">
          ...
          <ul class="ms-0 fa-ul">
            ...
            <li><div><i class="mt-2">${{$item->price}}</i></div></li>
            <li><div class="mt-2"><span>{{$item->description}}</span></div></li>
          </ul>
          <div class="mt-4 float-end">
            <input class="form-check-input" type="checkbox" id="event_active" name="event_active" onclick="activeId(this, {{$item->id}})" {!!$item->active==1? 'checked':''!!}>
            <a class="btn btn-info btn-xs" href="{{route('admin.event.edit', ['id'=>$item->id])}}"><i class="fas fa-pencil-alt me-1"></i>Edit</a>
  ...
  @if ($event_list->lastPage()>1)
    <div class="d-flex justify-content-end mt-4 me-4 mb-4">{{$event_list->links()}}</div>
  @endif
</div>
