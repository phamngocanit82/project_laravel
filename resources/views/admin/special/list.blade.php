<div class="card card-solid">
    ...
    @if (!empty($special_list))
      ...
      <div class="col-xxl-4 col-lg-6">
        <div class="project-box mb-4">
          ...
          <ul class="ms-0 fa-ul">
            <li><div><h5>{{$item->item}}</h5></div></li>
            <li><div><h6 class="mt-2">{{$item->title}}</h6></div></li>
            <li><div><i class="mt-2">{{$item->sub_title}}</i></div></li>
            <li><div class="mt-2"><span>{{$item->description}}</span></div></li>
          </ul>
          <div class="mt-4 float-end">
            <input class="form-check-input" type="checkbox" id="special_active" name="special_active" onclick="activeId(this, {{$item->id}})" {!!$item->active==1? 'checked':''!!}>
            <a class="btn btn-info btn-xs" href="{{route('admin.special.edit', ['id'=>$item->id])}}"><i class="fas fa-pencil-alt me-1"></i>Edit</a>
  ...
  @if ($special_list->lastPage()>1)
    <div class="d-flex justify-content-end mt-4 me-4 mb-4">{{$special_list->links()}}</div>
  @endif
</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Delete this special</h5>
...
<script>
    setTimeout(function () { 
      var Toast = Swal.mixin({
        ...
        Toast.fire({ 
          icon: 'error',  
          title:'Error edit about special!', 
          customClass: {
            title: 'mt-2',
          }
        })
