<div class="card card-solid">
    ...
    @if (!empty($gallery_list))
      ...
      <div class="col-xxl-4 col-lg-6">
        <div class="project-box mb-4">
          ...
          <ul class="ms-0 fa-ul">
            <li><div><h5 class="mt-2">{{$item->title}}</h5></div></li>
            <li><div class="mt-2"><span>{{$item->description}}</span></div></li>
          </ul>
          <div class="mt-4 float-end">
            <input class="form-check-input" type="checkbox" id="gallery_active" name="gallery_active" onclick="activeId(this, {{$item->id}})" {!!$item->active==1? 'checked':''!!}>
            <a class="btn btn-info btn-xs" href="{{route('admin.gallery.edit', ['id'=>$item->id])}}"><i class="fas fa-pencil-alt me-1"></i>Edit</a>
  ...
  @if ($gallery_list->lastPage()>1)
    <div class="d-flex justify-content-end mt-4 me-4 mb-4">{{$gallery_list->links()}}</div>
  @endif
</div>
...
<script>
    setTimeout(function () { 
      var Toast = Swal.mixin({
        ...
        Toast.fire({ 
          icon: 'error',  
          title:'Error edit about gallery!', 
          customClass: {
            title: 'mt-2',
          }
        })
